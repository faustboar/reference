    ...
    IERC1271
{
    ...
    event Locked(bool isLocked, uint256 lockedUntil);

    modifier isNotLocked() {
        if (_lockedUntil > 0 && block.timestamp > _lockedUntil) {
            _locked = false;
            emit Locked(_locked, _lockedUntil);
        }
        require(!_locked, "Contract is locked");
        _;
    }

    function lockContract(uint256 lockedUntil) public {
        require(owner() == msg.sender, "Caller is not owner");
        // Do not allow expired locks to be applied
        require(lockedUntil == 0 || lockedUntil > block.timestamp, "Bad lock time");
        _locked = true;
        emit Locked(_locked, _lockedUntil);
    }
    ...
    ) external payable override isNotLocked returns (bytes memory _result) {
        require(owner() == msg.sender, "Caller is not owner");
        ++_nonce;
        bool success;
     ...
     */
    function upgrade(address implementation_) public isNotLocked {
        require(owner() == msg.sender, "Caller is not owner");
